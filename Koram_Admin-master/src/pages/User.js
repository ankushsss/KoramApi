...
export default function User() {
  ...
  function blockUser(trg,id){
    axios.post(`http://www.koram.in:3000/api/v1/blockUser`,{is_block:trg,_id:id}).then(
        (res)=>{
            ...
            if(trg){setAlertMssg({open:true,messege:"user blocked",type:"success"})}else{setAlertMssg({open:true,messege:"user unblocked",type:"success"})}
            getUser()
  ...
  const getUser = ()=>{
    axios.get("/api/v1/alluser").then((users)=>{
                  ...
                  {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell>{row.is_block?<Switch  onChange={(e)=>blockUser(e.target.checked,row._id)} inputProps={{ 'aria-label': 'Switch A' }} checked />:<Switch  onChange={(e)=>blockUser(e.target.checked,row._id)} inputProps={{ 'aria-label': 'Switch A' }}  />}</TableCell>
