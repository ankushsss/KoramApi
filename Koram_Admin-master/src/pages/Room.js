...
export default function Room() {
  ...
  const getRoom = ()=>{
    axios.get("/api/v1/chatrooms").then((room)=>{
 ...
 useEffect(()=>{
  ...
  const uploadImage = () =>{
    ...
    console.log(form,file,imageId)
      axios.post("/api/v1/imageUploadRoom",form).then((room)=>{
        setImagePopUp(false)
        getRoom()
        // SETROOMLIST(room.data)
    })
                      ...
                      >
                        ...
                        <TableCell component="th" scope="row" padding="none">
                          <Stack direction="row" alignItems="center" spacing={2}>
                            <Avatar style={{cursor:"pointer"}} onClick={()=>setImageValue(row._id)} alt={row.name} src={`http://koram.in:3000/${row.image}`} />
